...
- (void)handleDragWhileLoading {
    if ([self isScrolledOverThreshold] || [self isScrolledToVisible]) {
        ...
        if (self.position == AllAroundPullViewPositionBottom) {
            ...
            self.scrollView.contentInset = UIEdgeInsetsMake(0, MIN(self.scrollView.contentOffset.x, kSidePullViewWidth), 0, 0);
        } else if (self.position == AllAroundPullViewPositionRight) {
            self.scrollView.contentInset = UIEdgeInsetsMake(0, 0, 0, MIN(-self.scrollView.contentOffset.x, kSidePullViewWidth));
        }
    }    
}
